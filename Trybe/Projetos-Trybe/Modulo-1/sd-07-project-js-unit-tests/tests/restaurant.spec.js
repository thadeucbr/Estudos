...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    let objetoQualquer = {};
    let input = createMenu(objetoQualquer);
    let result = createMenu(objetoQualquer);
    ...
    result = Object.keys(createMenu({food: {}, drink: {}}).fetchMenu);
    assert.deepStrictEqual(createMenu({food:{}, drink: {}}), result);
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    const objetoRetornado = createMenu({ food: {}, drink: {}});
    objetoRetornado.fetchMenu
    input = Object.is(objetoRetornado.fetchMenu, objetoRetornado.fetchMenu);
    assert.strictEqual(input, true)
    // TESTE 4: Verifique que 'objetoRetornado.consumption', após a criação do menu, retorna um array vazio.
    objetoRetornado.consumption // Retorno: []
    assert.strictEqual(objetoRetornado, []);
